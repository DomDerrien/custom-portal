define(
	...
	function(dateLocale) {
		...
		var _formatDateTime = function(value) {
			if (!value)
				return '';
			return dateLocale.format(new Date(value), { datePattern: 'MMM d, yyy', timePattern: 'H:mm z' });
		};

		var _formatDate = function(value) {
			if (!value)
				return '';
			return dateLocale.format(new Date(value), { selector: 'date', datePattern: 'MMM d, yyyy' });
		};

		var _formatTime = function(value) {
			if (!value)
				return '';
			return dateLocale.format(new Date(value), { selector: 'time', timePattern: 'H:mm z' });
		};
