define(
    ...
    function(common, ioQuery, dom, construct, registry, on, style, string, Cache, JsonRest, Memory, Observable, dbootstrap, DropDownButton, DropDownMenu, MenuItem, MenuSeparator) {
        ...
        var _buildCategoryPanes = function(insertionPoint, user) {
            var fluidRow = construct.create('div', {
                'class': 'row-fluid categories'
            }, insertionPoint),
                fluidRowSpanNb = 0;

            _categoryStore.query({
                ownerId: user.id
            }, {
                sort: [{
                    attribute: 'order'
                ...
                var table = construct.create('table', {
                    id: 'catBox-' + idx,
                    'class': (category.width === null ? 'span2' : category.width) + ' category',
                ...
                menu.addChild(new MenuItem({
                    ...
                    onClick: function() {
                        var orderList = registry.byId('setCatOrder'),
                            optionList = orderList.get('options'),
                            idx = 1;
                        if (optionList.length !== _categoryNb + 1) { // +1 because of the default option
                            while (idx < _categoryNb - 1) {
                                ...
                                optionList.push({
                                    value: '' + idx,
                                    label: 'Position: ' + idx
                                }); // Be careful: value MUST be a string
                            }
                            optionList.push({
                                value: '' + _categoryNb,
