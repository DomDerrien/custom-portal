...
/**
 * Holder of relevant data for scrolling that parses Range header from
 * HttpServletRequest.
 * 
 * Degraded formats from the standard startRow-endRow are accepted such as
 * startRow- and startRow. {@link InvalidRangeException} may be raised should
 * boundaries not be parsable.
 * 
 */
public class Range {
	...
	/**
	 * Passed on to service and DAO but not used for querying, count is used
	 * instead, used to potentially raise an InvalidRangeException within the
	 * DAO should the total not fit the requested boundaries
	 */
	private Integer endRow;

	/**
	 * Output value to inform the UI on the total instances available for the
	 * set perimeter
	 */
	...
	/**
	 * Method to form paging response header. may raise a
	 * {@link InvalidRangeException} in case of inconsistent boundaries. <br/>
