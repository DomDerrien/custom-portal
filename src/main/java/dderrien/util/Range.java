...
/**
 * Holder of relevant data for scrolling that parses Range header from HttpServletRequest. <br/>
 * Degraded formats from the standard startRow-endRow are accepted such as startRow- and startRow. {@link InvalidRangeException} may be raised should boundaries not be parsable.
 * 
 */
public class Range {
    ...
    /**
     * Passed on to service and DAO but not used for querying, count is used instead, used to potentially raise an InvalidRangeException within the DAO should the total not fit the
    ...
    public static Range parse(String rangeHeader) {
        ...
        try {
            ...
            // at this stage startRow < 0 cannot arise : minus sign and dash are the same !
    ...
    /**
     * Method to form paging response header. may raise a {@link InvalidRangeException} in case of inconsistent boundaries. <br/>
    ...
    public int getResponseStatus() {
        if (listSize == 0)
            return 204; // HttpStatus.SC_NO_CONTENT;

        if (listSize < this.total)
            return 206; // HttpStatus.SC_PARTIAL_CONTENT;
